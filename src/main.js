...
// ---- haptics (no-op on iOS; works on Android) ------------------------------
...
function buzzerZoneContent() {
  ...
  if (state.phase === 'revealed' && cur) {
    if (state.autoMissed) {
      return `<button id="buzzer" class="bigplay next"><span class="buzz-label">NEXT &#9654;</span></button>`;
    }
    return `
      <div class="bigplay split" role="group" aria-label="Grade your guess">
        <button id="missBtn" class="half half-miss"><span>MISSED</span></button>
        <button id="gotBtn" class="half half-got"><span>GOT IT</span></button>
      </div>`;
  }
  if (state.phase === 'buzzed' && cur) {
    return `<button id="buzzer" class="bigplay reveal"><span class="buzz-label">REVEAL</span></button>`;
  }
  if (state.phase === 'playing' && cur) {
    return `<button id="buzzer" class="bigplay buzz" ${state.busy ? 'disabled' : ''}>
      ${state.busy ? '<span class="spinner"></span>' : '<span class="buzz-label">BUZZ</span>'}
    </button>`;
  }
  if (state.phase === 'paused' && cur) {
    return `<button id="buzzer" class="bigplay resume"><span class="buzz-label">&#9654;</span></button>`;
  }
  // idle
  if (state.tracks.length === 0) return '';
  return `<button id="buzzer" class="bigplay start"><span class="buzz-label">START</span></button>`;
}

function controlsHtml() {
  const canPause = state.phase === 'playing' && !state.busy;
  const pauseBtn = `
    <button id="pauseBtn" class="bar-btn" ${canPause ? '' : 'disabled'}>
  ...
  return `
    <div class="bottombar">
      <div class="bottombar-inner">
        ${pauseBtn}
        <button id="removeBtn" class="bar-btn bar-x" ${canRemove ? '' : 'disabled'} aria-label="Remove this song" title="Remove this song">
        ...
        <button id="filterToggle" class="bar-btn filter-toggle ${
...
let firstPaint = true;
function renderGame() {
  const fpClass = firstPaint ? ' first-paint' : '';
  const revealClass =
  ...
  mount(`
    ${brandHeader()}
    ${statsRow()}
    <div class="info-box card fade-swap${fpClass}${revealClass}">${infoBoxContent()}</div>
    <div id="errorArea"></div>
    <div class="buzzer-zone">
      ${buzzerZoneContent()}
    </div>
    ${controlsHtml()}
  `);
  ...
  // wire up — the buzzer is the single primary control; it dispatches by phase
  on('buzzer', 'click', onBuzzerTap);
  on('gotBtn', 'click', onGotIt);
  on('missBtn', 'click', onMissed);
  on('pauseBtn', 'click', onPause);
  on('filterToggle', 'click', onToggleWrongOnly);

  // ripple + haptic on tap
  const bp = $('buzzer');
  if (bp) bp.addEventListener('pointerdown', (e) => { addRipple(bp, e); haptic(18); });
  const half1 = $('missBtn');
  const half2 = $('gotBtn');
  if (half1) half1.addEventListener('pointerdown', (e) => { addRipple(half1.parentElement, e); haptic(18); });
  if (half2) half2.addEventListener('pointerdown', (e) => { addRipple(half2.parentElement, e); haptic(18); });
  on('pauseBtn', 'pointerdown', () => haptic(10));
  on('filterToggle', 'pointerdown', () => haptic(10));
  on('removeBtn', 'click', onRemoveTap);
  on('removeBtn', 'pointerdown', () => haptic(10));
